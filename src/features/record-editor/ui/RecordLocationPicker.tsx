...
import { Check, LocateFixed, MapPin, Navigation, ShieldAlert, X } from 'lucide-react';
...
export function RecordLocationPicker({
  ...
  const [draftLocation, setDraftLocation] = useState<RecordLocation | null>(selectedLocation ?? null);
  ...
  const handleOpenSettings = useCallback(async () => {
    const kind = errorReason === 'services-disabled' ? 'services' : 'app';
    await openNativeLocationSettings(kind).catch(() => undefined);
  }, [errorReason]);
  ...
  return (
    ...
    >
      ...
      <div className="min-h-0 overflow-y-auto px-4 pb-[calc(16px+env(safe-area-inset-bottom))] pt-4">
        ...
        {errorReason && (
          <section className="mt-3 rounded-[18px] border border-border-primary bg-primary-light/25 p-4" data-record-location-error={errorReason}>
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-[14px] font-extrabold text-ww-ink">{t(`location.errors.${errorReason}.title`)}</div>
            ...
            <div className="mt-3 flex gap-2">
              <AppButton className="flex-1" onClick={() => void handleLocate()} size="compact" variant="secondary">
                {t('location.retry')}
              </AppButton>
              {canOpenSettings && (errorReason === 'permission-denied' || errorReason === 'services-disabled') && (
                <AppButton className="flex-1" onClick={() => void handleOpenSettings()} size="compact">
        ...
        {draftLocation && (
          <section className="mt-3 rounded-[18px] border border-primary bg-white p-4 shadow-ww-xs" data-record-location-selected>
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-[14px] font-extrabold text-ww-ink">{t('location.currentPosition')}</div>
                <div className="mt-0.5 truncate font-number text-[11px] font-semibold text-ww-soft">
                  ...
                  {' · '}
                  {t('location.accuracy', { meters: Math.round(draftLocation.accuracy) })}
                </div>
              </div>
              <Check aria-hidden="true" className="text-primary-deep" size={20} strokeWidth={2.2} />
            </div>
            <label className="mt-3 block">
              <span className="mb-1.5 block text-[12px] font-bold text-ww-soft">{t('location.nameLabel')}</span>
              <input
                ...
                maxLength={120}
                onChange={event => setDraftLocation(current => current
        ...
        <button
          ...
          type="button"
        >
          <span className="flex h-8 w-8 items-center justify-center rounded-[11px] bg-surface-subtle text-ww-soft"><X size={16} /></span>
          <span className="flex-1">{t('location.doNotRecord')}</span>
          {draftLocation === null && <Check aria-hidden="true" className="text-primary-deep" size={18} />}
        </button>

        {!canOpenSettings && errorReason === 'permission-denied' && (
          <p className="mt-3 text-[11px] font-semibold leading-5 text-ww-soft">{t('location.webPermissionGuide')}</p>
        )}
        ...
        <AppButton
          ...
          onClick={() => {
            ...
            onConfirm(draftLocation
              ? { ...draftLocation, ...(normalized ? { name: normalized } : { name: undefined }) }
              : null);
          }}
          size="large"
        >
