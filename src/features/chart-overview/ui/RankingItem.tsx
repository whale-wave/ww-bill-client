...
export const RankingItem: FC<{ item: ChartOverviewRankingItem; onClick?: () => void }> = ({ item, onClick }) => {
  ...
  return (
    <button
      ...
      type="button"
    >
      <span className={cn('flex h-[34px] w-[34px] shrink-0 items-center justify-center rounded-full bg-[rgba(111,194,220,0.16)] text-primary-deep')}><CategoryIcon categoryName={item.category.name} iconKey={item.category.icon} iconType={item.category.iconType} textIconEnabled={item.category.textIconEnabled} textIconIndex={item.category.textIconIndex} size={16} /></span>
