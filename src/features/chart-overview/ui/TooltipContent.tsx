        ...
        {list.map(item => (
          <div className="flex min-w-0 items-center gap-2 rounded-[12px] bg-ww-surface-tint px-2 py-1.5" key={item.id}>
            <span className="ww-chart-tooltip-category flex h-7 w-7 shrink-0 items-center justify-center rounded-full">
              <CategoryIcon categoryName={item.category.name} iconKey={item.category.icon} iconType={item.category.iconType} textIconEnabled={item.category.textIconEnabled} textIconIndex={item.category.textIconIndex} size={15} />
            </span>
