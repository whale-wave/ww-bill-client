...
export const ChartDisplaySwitch: FC<ChartDisplaySwitchProps> = ({ compact = false, value, onChange }) => {
  ...
  return (
    <div
      ...
      className={`ww-chart-display-switch inline-flex items-center border p-[2px] ${compact ? 'w-[88px] origin-right' : ''}`}
    ...
    >
      {options.map(option => (
        <button
          ...
          className={`ww-chart-display-switch__option relative flex h-full items-center justify-center border-0 bg-transparent font-bold transition-colors ${compact ? 'min-w-0 flex-1 px-1 text-[10px] leading-4' : 'min-w-[52px] px-3 text-[12px] leading-[18px]'} ${option.value === value ? 'ww-chart-display-switch__option--active' : ''}`}
