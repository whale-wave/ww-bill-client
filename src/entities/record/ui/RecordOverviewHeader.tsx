...
export const RecordOverviewHeader: FC<RecordOverviewHeaderProps> = ({
  ...
  const titleClassName = cn(
    'absolute top-[5px] text-xl font-bold text-[#323334]',
  ...
  return (
    <div
      className={cn(styles.top, 'record-detail-top relative h-[182px] w-full shrink-0')}
      data-record-overview-header=""
      data-testid={testId}
    >
      {renderTitle(titleClassName)}
      <div className="absolute left-5 top-[50px] flex flex-col">
      ...
      {metrics.map((metric, index) => (
        <div
          className={cn(
            'absolute top-[50px] flex flex-col',
            index === 0 ? 'left-[121px]' : 'left-[230px]',
          )}
          key={metric.key}
        >
          <div className="text-sm font-normal text-[#333333]">{metric.label}</div>
          <div className="relative h-[37px] w-24 text-base font-normal text-[#333333]">
            <div className="absolute bottom-0" data-testid={metric.testId}>{metric.value}</div>
          </div>
        </div>
      ))}
      ...
      <div className="absolute bottom-0 left-[10px] right-[10px] h-[68px]">
        ...
        >
          {shortcuts.map(shortcut => (
            <button
              className="flex shrink-0 grow flex-col items-center justify-center border-0 bg-transparent text-sm text-[#333333]"
