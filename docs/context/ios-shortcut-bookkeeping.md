...
4. 快捷记账固定写入个人默认账本。用户仍可在原编辑器修改收支类型、分类、金额、备注、日期和关联资产账户；已配置的默认资产账户会预选。
5. 用户点击原记账编辑器的完成按钮后，仍调用草稿确认接口创建正式记录，而不是普通新增记录接口。
6. 用户在原编辑器取消时放弃草稿，使确认链接失效；服务端保留 OCR 原文和候选字段，供后续排查识别问题。
...
- 客户端的 OCR 原文和 review code 只保存在当前原记账页的路由状态中，不写入 React Query 持久化缓存；服务端另在 `shortcut_draft` 中保留 OCR 原文供排查。领取 code 只在草稿领取前临时进入当前标签页的 `sessionStorage`，领取成功、确认或放弃后立即清除。
