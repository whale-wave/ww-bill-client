...
Create `src/shared/ui/page-loading-state/PageLoadingState.tsx` + `page-loading-state.scss` + `index.ts`、派生 `src/assets/brand/whale-loading.png`（`sips -Z 120 src/assets/brand/whale-logo-transparent.png --out src/assets/brand/whale-loading.png`，验 <50KB）；Modify `src/shared/ui/index.ts`、`LedgerRecoveryPage.tsx`、`features/ledger-collaboration/ui.tsx`、`features/household/ui/HouseholdPageState.tsx`；i18n `recovery.empty`；Test `test/shared/ui/page-loading-state.test.tsx`、更新 `test/features/ledger-collaboration-ui.test.ts:66-73`
